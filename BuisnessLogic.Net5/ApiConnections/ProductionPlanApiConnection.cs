...
namespace BlazorBuisnessLogic.Net5.ApiConnections
{
    public class ProductionPlanApiConnection : IDocumentListAPI
    {
        ...
        public async Task<List<ProductionPlanColumn>> GetProductionPlanLockCells(Guid ProductionPlanPublicIndentifier)
        {
            HttpResponseMessage response = await HttpClient.GetAsync(
                UriGenneratore.GennreadURL($"/api/ProductionPlanLockGet?ProductionPlanPublicIdentifier={ProductionPlanPublicIndentifier}"));
            if (await ErrorMessage.FaildToConnect(response))
            {
                return await response.ReadFromJasonAsync<List<ProductionPlanColumn>>();
            }
            return null;
        }
    }
