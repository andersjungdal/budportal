...
namespace BlazorBuisnessLogic.Net5.ApiConnections
{
    public class RawBidApiConnection: IDocumentListAPI
    {
        ...
        public async Task<List<RawBidColumn>> GetRawBidLockCells(Guid RawBidPoblickIndentifire)
        {
            HttpResponseMessage response = await HttpClient.GetAsync(
                UriGenneratore.GennreadURL($"/api/RawBidLockGet?RawBidPublicIdentifier={RawBidPoblickIndentifire}"));
            if (await ErrorMessage.FaildToConnect(response))
            {
                return await response.ReadFromJasonAsync<List<RawBidColumn>>();
            }
